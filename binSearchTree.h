...
private:
	struct Node
	{
		...
		Node *parent_ = nullptr;        // parent added for traversal up tree
	};
...
public:
	 ...
	 * not needed for anything, testing only */
	...
	public:
		...
		/* used to call end()
		 * this was an attempt at a workaround, pretty sure
		 * this will lead to some unforeseen issue */
		Iterator()
		 ...
		 * returns starting pointer of iterator based on Traversal order specifed */
		Iterator getStart()
		{
			if (order_ == Traversal::PreOrder)
			{
	...
	private:
		...
		/*recursive method called by post-increment operator
		 * for traversal order InOrder */
		void nextInOrder()
		{
			if (iter_->right_)
			{
		...
		/*recursive method called by post-increment operator
		 * for traversal order PostOrder.
		 * TODO nextPostOrder() needs to be fixed to check the following condition:
		 * "does my right child, have a left child"
		 * nextPostOrer was skipping a branch in Gradescope*/
		void nextPostOrder()
		{
		...
		/* not needed for anything, used for testing only*/
		void goToRightSibling()
		{
			if (iter_->parent_->right_.get() && iter_ != iter_->parent_->right_.get())
			{
				iter_ = iter_->parent_->right_.get();
			}
		}
		...
		Node *iter_ = nullptr;      // iterator used for iterating tree
		const Traversal order_;     // order_ is needed for insert()
	};
...
private:
	 ...
	 *
	 * 	insert() needed to be modified quite a bit to insert parent properly.
	 * 	child (i.e. left_ and right_ ) is a unique_ptr and parent is a reg. node ptr
	 */
...
/*  just a reference diagram to help
 *
 *                                 (7)
 *                              /       \
 *                          (5)           (10)
